...
public class BaseScreen {
	...
	throws ScreenException {
		/*
		if (selectedBrowser.equalsIgnoreCase(Constants.BROWSER_CHROME)) { try
		{ chromeService = new ChromeDriverService.Builder()
		.usingDriverExecutable(new File(getChromeLocation()))
		.usingAnyFreePort().build();
		
		log.info("LAUNCHING GOOGLECHROME"); driver = new
		ChromeDriver(chromeService); windowResize();
	...
	directory = System.getProperty("user.dir"); String targetDirectory =
	getChromeFile(); String location = directory + targetDirectory; return
	location; }
	...
	public String getChromeFile() { if
	(System.getProperty("os.name").startsWith(Constants.WINDOWS_OS)) {
	log.info("WINDOWS MACHINE FOUND"); return Constants.CHROME_DIRECTORY +
	"/chromedriver.exe"; } else if
	(System.getProperty("os.name").startsWith(Constants.LINUX_OS)) {
	log.info("LINUX MACHINE FOUND"); return Constants.CHROME_DIRECTORY +
	"/chromedriver"; } else if
	(System.getProperty("os.name").startsWith(Constants.MAC_OS)) {
	log.info("MAC MACHINE FOUND"); return Constants.CHROME_DIRECTORY +
	"/chromedriver"; } else { throw new
	NullPointerException("PLATFORM NOT FOUND");
	...
	public void registration(String methodName){
		try {
